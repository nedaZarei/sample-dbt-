...
with quarterly_cashflows as (
    ...
    select
        cf.fund_id,
        cf.portfolio_id,
        -- Cast to DATE for consistent cross-database behavior
        cast(date_trunc('quarter', cf.cashflow_date) as date) as quarter_start,
    ...
    group by
        ...
        cast(date_trunc('quarter', cf.cashflow_date) as date),
...
quarterly_valuations as (
    ...
    from (
        select
            ...
            row_number() over (
                partition by v.portfolio_id, cast(date_trunc('quarter', v.valuation_date) as date)
...
lp_report as (
    ...
    left join quarterly_valuations qv
        ...
        and cast(date_trunc('quarter', qv.valuation_date) as date) = qc.quarter_start
